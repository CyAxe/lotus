...
impl LuaRunTime<'_, '_> {
    ...
    pub fn encoding_func(&self) {
        self.lua.globals()
            .set(
                "base64encode",
                self.lua.create_function(|_, data: String| Ok(base64::encode(data)))
                    .unwrap(),
            )
            .unwrap();
        ...
        self.lua.globals()
            .set(
                "base64decode",
                self.lua.create_function(|_, data: String| Ok(base64::decode(data).unwrap()))
                    .unwrap(),
            )
            .unwrap();
    }
    ...
    pub fn http_func(&self,target_url: Option<&str>) {
        self.lua.globals()
            .set(
                "JOIN_SCRIPT_DIR",
                self.lua.create_function(|c_lua, new_path: String| {
        ...
        let log_info = self.lua
            .create_function(|_, log_msg: String| {
                info!("{}", log_msg);
                Ok(())
            })
            .unwrap();
        let log_warn = self.lua
            .create_function(|_, log_msg: String| {
                warn!("{}", log_msg);
                Ok(())
            })
            .unwrap();
        let log_debug = self.lua
            .create_function(|_, log_msg: String| {
                debug!("{}", log_msg);
                Ok(())
            })
            .unwrap();
        let log_error = self.lua
            .create_function(|_, log_msg: String| {
                error!("{}", log_msg);
                Ok(())
            })
        ...
        self.lua.globals()
            .set(
                "sleep",
                self.lua.create_async_function(|_, time: u64| async move {
        ...
        self.lua.globals()
            .set(
                "readfile",
                self.lua.create_function(|_ctx, file_path: String| {
    ...
    pub fn get_matching_func(&self) {
        ...
        self.lua.globals()
            .set(
                ...
                self.lua.create_function(|_, payload: String| Ok(css_selector(&payload)))
                    .unwrap(),
            )
            .unwrap();
        ...
        self.lua.globals()
            .set(
                "html_parse",
                self.lua.create_function(|_, (html, payload): (String, String)| {
        ...
        self.lua.globals()
            .set(
                "html_search",
                self.lua.create_function(|_, (html, pattern): (String, String)| {
